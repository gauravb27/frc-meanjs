...
(function() {
	...
	describe('EntriesController', function() {
		...
		it('$scope.find() should create an array with at least one article object fetched from XHR', inject(function(Entries) {
			// Create sample article using the Entries service
			...
			var sampleArticles = [sampleEntry];
		...
		it('$scope.findOne() should create an array with one article object fetched from XHR using a articleId URL parameter', inject(function(Entries) {
			...
			// Set GET response
			$httpBackend.expectGET(/articles\/([0-9a-fA-F]{24})$/).respond(sampleEntry);
			...
			// Test scope value
			expect(scope.article).toEqualData(sampleEntry);
		}));
		...
		it('$scope.create() with valid form data should send a POST request with the form input values and then locate to new object URL', inject(function(Entries) {
			...
			var sampleArticlePostData = new Entries({
				title: 'An Article about MEAN',
				content: 'MEAN rocks!'
			});

			// Create a sample article response
			var sampleArticleResponse = new Entries({
		...
		it('$scope.update() should update a valid article', inject(function(Entries) {
			// Define a sample article put data
			var sampleArticlePutData = new Entries({
				_id: '525cf20451979dea2c000001',
				title: 'An Article about MEAN',
				content: 'MEAN Rocks!'
			});
		...
		it('$scope.remove() should send a DELETE request with a valid articleId and remove the article from the scope', inject(function(Entries) {
